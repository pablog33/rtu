		...
		if ((err_accept = netconn_accept(conn, &newconn)) == ERR_OK) {
			...
			while ((err_recv = netconn_recv(newconn, &buf)) == ERR_OK) {
				do {
					...
					if ((err_dataBuf = netbuf_data(buf, &pHMIData,
							&uiLenRecvData)) != ERR_OK) {
						lDebug(Error, "Error en funcion NETCONN -netbuf_data-");
						prvNetconnError(err_dataBuf);
						iServerStatus = ERROR_NETCONN;
						break;
					}

					if ((iServerStatus = NetValuesReceivedFromHMI(pHMIData,
							&HMICmd, uiLenRecvData)) != ERR_OK) {
						break;
					}
					...
					if ((err_send = netconn_write(newconn, RTUDataTx.buffer,
							sizeof(RTUDataTx.buffer), NETCONN_COPY)) != ERR_OK) {
						lDebug(Error, "Error en funcion NETCONN -netbuf_data-");
						prvNetconnError(err_send);
						iServerStatus = ERROR_NETCONN;
						break;
					}
			...
			if (err_recv) {
				lDebug(Error, "Error en funcion NETCONN -netconn_recv-");
				prvNetconnError(err_recv);
			}
