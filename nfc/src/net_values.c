...
/*-----------------------------------------------------------*/
int16_t NetValuesReceivedFromHMI(HMIData_t *HMIData, HMICmd_t *HMICmd,
...
{ /*	HMI_NETVAR_SIZE + 1: Para tener en cuenta el fin de cadena en caracteres '\0' 	*/
	...
	if ((iServerStatus = prvFormatoTramaRecv(uiLenDataRecv)) == ERR_OK) {
		/*	-- clientID --*/
		if (!strncmp(HMIData->clientId, "SM13;", HMI_NETVAR_SIZE)) {
			HMICmd->clientID = eSigned;
		} else {
			lDebug(Error, "error- HMICmd->ClientID", HMI_NETVAR_SIZE);
			HMICmd->clientID = eUnsigned;
			iServerStatus = ERROR_TRAMA_CLIENTE;
		}
		...
		if (!iServerStatus) {
			...
			if (HMICmd->velCmdArm > 8 || HMICmd->velCmdArm < 0) {
				lDebug(Error, "error- HMICmd->velCmdArm");
				iServerStatus = ERROR_TRAMA_DATO;
			} /* 0x82 */
			if (HMICmd->velCmdPole > 8 || HMICmd->velCmdPole < 0) {
				lDebug(Error, "error- HMICmd->velCmdPole");
				iServerStatus = ERROR_TRAMA_DATO;
			} /* 0x82 */
			...
			/*	-- mode --	*/
			if (!strncmp(HMIData->mode, "STOP;", HMI_NETVAR_SIZE)) {
				HMICmd->mode = eStop;
			} else if (!strncmp(HMIData->mode, "FRUN;", HMI_NETVAR_SIZE)) {
				HMICmd->mode = eFree_run;
			} else if (!strncmp(HMIData->mode, "AUTO;", HMI_NETVAR_SIZE)) {
				HMICmd->mode = eAuto;
			} else if (!strncmp(HMIData->mode, "LIFT;", HMI_NETVAR_SIZE)) {
				HMICmd->mode = eLift;
			} else {
				lDebug(Error, "error- HMICmd->mode");
				iServerStatus = ERROR_TRAMA_DATO;
			}

			/*	--	freeRunAxis --	*/
			if (!strncmp(HMIData->freeRunAxis, "ARM_;", HMI_NETVAR_SIZE)) {
				HMICmd->freeRunAxis = eArm;
			} else if (!strncmp(HMIData->freeRunAxis, "POLE;",
					HMI_NETVAR_SIZE)) {
				HMICmd->freeRunAxis = ePole;
			} else {
				lDebug(Error, "error- HMICmd->freeRunAxis");
				iServerStatus = ERROR_TRAMA_DATO;
			}

			/*	--	freeRunDir --	*/
			if (!strncmp(HMIData->freeRunDir, "CW__;", HMI_NETVAR_SIZE)) {
				HMICmd->freeRunDir = eCW;
			} else if (!strncmp(HMIData->freeRunDir, "CCW_;",
					HMI_NETVAR_SIZE)) {
				HMICmd->freeRunDir = eCCW;
			} else {
				lDebug(Error, "error- HMICmd->freeRunDir");
				iServerStatus = ERROR_TRAMA_DATO;
			}

			/*	-- ctrlEn --	*/
			if (!strncmp(HMIData->ctrlEn, "CTLE;", HMI_NETVAR_SIZE)) {
				HMICmd->ctrlEn = eEnable;
			} else if (!strncmp(HMIData->ctrlEn, "DCTL;", HMI_NETVAR_SIZE)) {
				HMICmd->ctrlEn = eDisable;
			} else {
				lDebug(Error, "error- HMICmd->ctrlEn");
				iServerStatus = ERROR_TRAMA_DATO;
			}

			/*	-- stallEn --	*/
			if (!strncmp(HMIData->stallEn, "STLE;", HMI_NETVAR_SIZE)) {
				HMICmd->stallEn = eEnable;
			} else if (!strncmp(HMIData->stallEn, "DSTL;", HMI_NETVAR_SIZE)) {
				HMICmd->stallEn = eDisable;
			} else {
				lDebug(Error, "error- HMICmd->stallEn");
				iServerStatus = ERROR_TRAMA_DATO;
			}

			/*	-- liftDir --	*/
			if (!strncmp(HMIData->liftDir, "LFUP;", HMI_NETVAR_SIZE)) {
				HMICmd->liftDir = eUp;
			} else if (!strncmp(HMIData->liftDir, "LFDW;", HMI_NETVAR_SIZE)) {
				HMICmd->liftDir = eDown;
			} else {
				lDebug(Error, "error- HMICmd->liftDir");
				iServerStatus = ERROR_TRAMA_DATO;
			}

			/*	-- setCal --	*/
			if (!strncmp(HMIData->setCal, "CAL_;", HMI_NETVAR_SIZE)) {
				HMICmd->setCal = eCal;
				lDebug(Info, "SET_CAL POLE: %i, ARM: %i", HMIData->posCmdPole,
						HMIData->posCmdArm);
			} else if (!strncmp(HMIData->setCal, "NOP_;", HMI_NETVAR_SIZE)) {
				HMICmd->setCal = eNop;
			} else {
				lDebug(Error, "error- HMICmd->setCal");
				iServerStatus = ERROR_TRAMA_DATO;
			}
